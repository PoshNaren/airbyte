...
class Stats(SendgridStreamOffsetPagination):
...
class CategoriesStat(SendgridStream):

    def __init__(self, start_date: str, **kwargs):
        super().__init__(**kwargs)
        if start_date == None:
            self._start_date = pendulum.now().subtract(days = 1).to_date_string()
    ...
    def request_params(self, **kwargs) -> MutableMapping[str, Any]:
        ...
        params = super().request_params(**kwargs)
        start_date = self._start_date
        params.update({"start_date": start_date, "end_date": start_date, "aggregated_by": "day", "limit": "9999999999"})
        return params

    def path(self, **kwargs) -> str:
        return "categories/stats/sums"
